...
**Generated by the test suite, not by hand.** These figures are asserted in
`src/engine/__tests__/score.test.ts`; if a weight changes, that test fails on
purpose and this table must be regenerated rather than left stale.

### Constant correction

An earlier draft of this table used a steerer-rise constant of 50 mm. That was
wrong. The rise from the top of the head tube to the centre of the stem's bar
clamp is:

```
topCapHeight + stemClampHeight / 2  =  10 + 40/2  =  30 mm
```

Every figure below is recomputed with 30 mm. The correction lowers the grip
point about 19 mm for a given spacer height, which is why the target stack here
(651 mm) is lower than the 670 mm used previously.

### The target is an achievable position, not a round number

Rather than inventing "grip reach 590, grip stack 670", the target is defined as
**a Race 56 with a 100 mm −6° stem and 20 mm of spacers** — grip reach
**594.1 mm**, grip stack **651.2 mm**. That makes the calibration
self-validating: the frame the target came from must win, and the test asserts
exactly that.
...
| Endurance 54 | 570 | 380 | 71.5 | 114 mm | 8 mm | 91.7 | excellent |
| Endurance 56 | 592 | 387 | 72.0 | 104 mm | 9 mm | 91.5 | excellent |
...
right reasons — a 127 mm stem at one end, negative spacers at the other — and an
unambiguous rejection when the front end is 61 mm too tall to slam away.

### An exchange rate worth knowing

A test written on the naive assumption that 20 mm of extra frame reach buys back
20 mm of stem **failed**. It does not. The stem sits at an angle and the spacer
height co-adjusts to hold the height, so the exchange rate falls out of the
linear system as

```
dStemLength = dFrameReach * sin(headTubeAngle) / cos(stemAngle)
```

which is 19.28 mm, not 20, at 73.5° and −6°. Small, but it is a systematic bias
in exactly the place where two adjacent sizes are being compared.

**What this evidence does not establish.** It shows the ranking is *ordered*
sensibly and that the engine reproduces its own reference. It does not show the
thresholds sit in the right places, because there is no ground truth here — only
the author's judgement. See Q5 in the product spec: the calibration set is 20
riders on bikes they already ride well, and the engine must rank their own bike
first.
