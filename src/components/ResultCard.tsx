...
export function ResultCard({
  ...
  return (
    <li className="rounded-[10px] border border-[var(--border)] bg-[var(--card)] p-4">
      <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1">
        <span className="font-medium">
          {frame.model} <span className="text-[var(--muted-foreground)]">·</span> {frame.size}
        </span>
        <span
      ...
      {explanation.mechanism ? (
        <p className="mt-1 text-sm text-[var(--muted-foreground)]">{explanation.mechanism}</p>
      ) : null}
      {explanation.requirement ? (
        <p className="mt-1 text-sm text-[var(--muted-foreground)]">{explanation.requirement}</p>
      ) : null}

      <dl className="tabular mt-3 flex flex-wrap gap-x-5 gap-y-1 text-xs text-[var(--muted-foreground)]">
